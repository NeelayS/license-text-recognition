...
def model(images, weight_decay=1e-5, is_training=True):
    ...
    with tf.variable_scope("feature_fusion", values=[end_points.values]):
        ...
        with slim.arg_scope(
            ...
            geo_map = (
                slim.conv2d(g[3], 4, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None)
                * FLAGS.text_scale
            )
            angle_map = (
                (
...
def loss(y_true_cls, y_pred_cls, y_true_geo, y_pred_geo, training_mask):
    ...
    tf.summary.scalar(
        "geometry_AABB", tf.reduce_mean(L_AABB * y_true_cls * training_mask)
    )
    tf.summary.scalar(
        "geometry_theta", tf.reduce_mean(L_theta * y_true_cls * training_mask)
    )
